...
(function(){
  ...
  let stage=null, scale=1, rot=0, panX=0, panY=0, startX=0, startY=0, isPanning=false, currentBlobUrl=null;
  ...
  function close(){ viewer.classList.remove('open'); viewerContent.innerHTML=''; stage=null; currentBlobUrl=null; }
  function open(node, blobUrl){
    viewerContent.innerHTML=''; viewerContent.appendChild(node);
    currentBlobUrl = blobUrl || null;
    if(currentBlobUrl){
      btnNewTab.onclick = ()=> window.open(currentBlobUrl, '_blank');
      btnDownload.href = currentBlobUrl;
    }else{
      btnNewTab.onclick = null;
  ...
  function openImage(url){
    ...
    viewerContent.innerHTML=''; viewerContent.appendChild(wrap);
